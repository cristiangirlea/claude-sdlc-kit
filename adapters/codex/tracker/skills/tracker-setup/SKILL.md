...
**Backend:** the user's request (default: `local`)
...
5. Run the read-only queries (`the `tracker-report` skill`) and confirm the picture is right.
...
- Note the chosen backend in `AGENTS.md` so future sessions do not have to re-discover it.
