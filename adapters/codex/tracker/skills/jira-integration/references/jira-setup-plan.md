...
**Verify:** `the `tracker-report` skill` produces a correct picture with `backend: "jira"` while all write paths are still disabled.
...
- [ ] `the `tracker-pick` skill` - JQL for ready work, unassigned first.
- [ ] `the `tracker-start` skill` - assign to self, transition to In Progress, create the branch named for the key.
- [ ] `the `tracker-sync` skill` - comment the PR link, transition to In Review.
- [ ] `the `tracker-comment` skill` - free-form note.
...
- [ ] A weekly `the `tracker-report` skill` for standup.
