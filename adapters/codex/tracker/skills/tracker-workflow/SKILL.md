...
| Command | Effect |
| --- | --- |
| `the `tracker-setup` skill` | Configure the backend for this repo |
| `the `tracker-pick` skill` | Show what to work on next, or create the item |
| `the `tracker-start` skill <id>` | `in-progress`, create the branch, link them |
| `the `tracker-sync` skill` | Push branch/PR/status from the local repo to the item |
| `the `tracker-comment` skill <id>` | Add a note to the item's log |
| `the `tracker-report` skill` | Standup-style summary of the current window |
