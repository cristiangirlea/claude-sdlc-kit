...
- **local:** `node "<absolute plugin resource root>/scripts/tracker.mjs" new "<title>" --type <type> --priority <P1..P4>`
...
2. **List `ready` items**, highest priority first.
   - local: `node "<absolute plugin resource root>/scripts/tracker.mjs" next`
