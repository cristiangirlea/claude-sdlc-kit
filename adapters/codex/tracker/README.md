...
| Procedure | Effect |
| --- | --- |
| `the `tracker-setup` skill [local\|jira]` | Configure the backend for this repo |
| `the `tracker-pick` skill [title]` | Recommend the next item, or capture a new one |
| `the `tracker-start` skill <id>` | Move to in-progress and create the linked branch |
| `the `tracker-sync` skill [id]` | Push branch/PR/status from the repo onto the item |
| `the `tracker-comment` skill <id> <text>` | Add a dated note to the item's log |
| `the `tracker-report` skill [days]` | Standup summary, cross-checked against git |
...
node "<absolute plugin resource root>/scripts/tracker.mjs" move TASK-1 in-progress
node "<absolute plugin resource root>/scripts/tracker.mjs" set TASK-1 branch=feat/TASK-1-saved-searches pr=123
node "<absolute plugin resource root>/scripts/tracker.mjs" comment TASK-1 "blocked on the auth decision"
node "<absolute plugin resource root>/scripts/tracker.mjs" report --days 7
node "<absolute plugin resource root>/scripts/tracker.mjs" next
```
