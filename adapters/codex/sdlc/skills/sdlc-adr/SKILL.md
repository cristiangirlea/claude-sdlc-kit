...
**Decision:** the user's request

Follow `adr-writing`.

1. **Check it deserves an ADR.** Hard to reverse - schema, dependency, public contract, auth or tenancy model, where data lives, sync/async boundary, deliberate accepted limitation? If a single PR could undo it in an afternoon, put it in `AGENTS.md` instead and say so.
