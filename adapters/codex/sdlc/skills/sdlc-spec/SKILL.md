...
2. **Ground it in the code.** Find the most similar existing feature and read it, so the spec matches how this repo actually works. Use the `codebase-explorer` role (references/agents/codebase-explorer.md) if the area is unfamiliar.
3. **Run the `spec-analyst` role (references/agents/spec-analyst.md)** with the request plus what you found. It returns the spec document.
