...
2. **Run the `qa-verifier` role (references/agents/qa-verifier.md)** so the raw output stays out of this conversation and only the signal comes back: build, type-check, lint, unit tests, integration tests, coverage on changed files.
