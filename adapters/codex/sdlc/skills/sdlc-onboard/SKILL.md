...
2. **Map one path.** Run the `codebase-explorer` role (references/agents/codebase-explorer.md) over one representative request or job, end to end, so `AGENTS.md` can state the layering rule accurately.
