...
<!-- Generated from src/skills/sdlc-workflow/SKILL.md by scripts/build.mjs. Edit the source, not this file. -->
...
Every piece of work gets an id before code is written - see the `tracker` plugin (`the `tracker-pick` skill`, `the `tracker-start` skill`). No id means no traceability from commit to reason.
...
Run `the `sdlc-spec` skill`. See the `spec-writing` skill.
...
Run `the `sdlc-plan` skill`. See `task-decomposition`. Hard-to-reverse decisions become ADRs - see `adr-writing`.
...
Run `the `sdlc-implement` skill`. See `tdd-workflow`. One slice at a time; parallel agents only for slices with disjoint file scopes.
...
Run `the `sdlc-review` skill`. See `code-review-standards`.

### 6. Verify
**Owner:** `qa-verifier`. **Artefact:** the gate table. **Gate:** the `definition-of-done` checklist passes.

Run `the `sdlc-verify` skill`. Green means observed green, not assumed.
...
Run `the `sdlc-ship` skill`. See `git-workflow` and `release-management`.

### 8. Learn
**Owner:** main thread. **Artefact:** an updated `AGENTS.md`, a new ADR, or a tightened checklist.
...
AGENTS.md                        the conventions an agent must know
```
...
| Stage | Command | Agent |
| --- | --- | --- |
| Spec | `the `sdlc-spec` skill` | `spec-analyst` |
| Plan | `the `sdlc-plan` skill` | `solution-architect`, `codebase-explorer` |
| Implement | `the `sdlc-implement` skill` | `test-author`, `task-implementer` |
| Fix a bug | `the `sdlc-bugfix` skill` | `debugger`, `test-author` |
| Review | `the `sdlc-review` skill` | `code-reviewer`, `security-auditor` |
| Verify | `the `sdlc-verify` skill` | `qa-verifier` |
| Ship | `the `sdlc-ship` skill` | `docs-scribe`, `release-manager` |
| Record a decision | `the `sdlc-adr` skill` | - |
| Adopt the kit in a repo | `the `sdlc-onboard` skill` | `codebase-explorer` |
| Where am I | `the `sdlc-status` skill` | - |
