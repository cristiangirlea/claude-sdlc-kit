...
**Slice:** $ARGUMENTS (if empty, take the next unchecked slice in the plan)
...
3. **Red.** Run the `test-author` agent to write the failing tests for this slice's acceptance criteria. Require the reported failure to be behavioural - an import error is not a red test.
4. **Green.** Run the `task-implementer` agent (or implement directly for a small slice) with an explicit file scope. Minimum code to pass; no scope creep; no weakening of tests.
5. **Refactor** under a green suite if the code needs it. No new behaviour in this step.
6. **Verify** with `/sdlc:verify` - build, lint, type-check, full test suite. Green means observed green.
