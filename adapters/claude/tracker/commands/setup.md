...
**Backend:** $ARGUMENTS (default: `local`)
...
3. Create one item to prove the loop works: `node "${CLAUDE_PLUGIN_ROOT}/scripts/tracker.mjs" new "Adopt the SDLC kit" --type chore --priority P3`.
...
5. Run the read-only queries (`/tracker:report`) and confirm the picture is right.
...
- Note the chosen backend in `CLAUDE.md` so future sessions do not have to re-discover it.
