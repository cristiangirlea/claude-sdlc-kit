...
**Spec:** {{ARGS}} (if empty, use the most recently modified file in `docs/specs/`)

Follow `task-decomposition`; the design work belongs to the {{AGENT:solution-architect}}.
...
2. **Understand the ground.** If the affected code is unfamiliar, run the {{AGENT:codebase-explorer}} over the relevant paths first and feed its map into the planning step.
3. **Run the {{AGENT:solution-architect}}** with the spec plus the exploration map. It returns: chosen design, rejected alternatives, file-level changes, ordered slices, test strategy, rollout, ADR candidates.
...
6. **Raise the ADR candidates** with the user explicitly. Anything hard to reverse gets `{{CMD:sdlc:adr}}` before implementation starts, not after.
