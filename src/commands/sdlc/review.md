...
**Target:** {{ARGS}} (empty = uncommitted changes plus commits on this branch versus the default branch)
...
2. **Run the {{AGENT:code-reviewer}}** over it.
3. **Run the {{AGENT:security-auditor}} in addition** when the diff touches any of: authentication, authorization, user input parsing, file handling, outbound HTTP, secrets or configuration, payments, personal data, or database migrations. When in doubt, run it - the cost is one agent, the alternative is a breach.
...
**Gate:** no unaddressed blockers or majors before `{{CMD:sdlc:ship}}`.
