...
1. **Gather the input.** If the argument is a tracker id, pull the item (`{{CMD:tracker:show}} <id>`). If it is a path, read it. If it is prose, use it as-is.
2. **Ground it in the code.** Find the most similar existing feature and read it, so the spec matches how this repo actually works. Use the {{AGENT:codebase-explorer}} if the area is unfamiliar.
3. **Run the {{AGENT:spec-analyst}}** with the request plus what you found. It returns the spec document.
