...
**Scope:** {{ARGS}} (default: all gates)

1. **Find the real commands.** Read `{{MEMORY}}`, `Makefile`, `package.json`, `justfile`, and the CI workflow. CI is the definition of green - if the README disagrees with CI, believe CI.
2. **Run the {{AGENT:qa-verifier}}** so the raw output stays out of this conversation and only the signal comes back: build, type-check, lint, unit tests, integration tests, coverage on changed files.
