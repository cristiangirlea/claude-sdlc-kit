...
**Input:** {{ARGS}} (first token is the id; the rest is the note. With no id, use the id in the branch name.)
...
3. **Post it.**
   - local: `node "{{PLUGIN_ROOT}}/scripts/tracker.mjs" comment <ID> "<text>"` - the entry is dated and prepended to the item's Log.
