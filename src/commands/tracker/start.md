...
1. **Read the item.** Show the user the title, description and acceptance criteria before anything changes. If it has no acceptance criteria and is not trivial, run `{{CMD:sdlc:spec}}` first - starting on an item nobody has scoped is how a day disappears.
...
6. **Report** the branch, the item, the acceptance criteria, and the next command (`{{CMD:sdlc:spec}}` if there is no spec, otherwise `{{CMD:sdlc:plan}}` or `{{CMD:sdlc:implement}}`).
