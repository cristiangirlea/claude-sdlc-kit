...
- **local:** `node "{{PLUGIN_ROOT}}/scripts/tracker.mjs" new "<title>" --type <type> --priority <P1..P4>`
...
2. **List `ready` items**, highest priority first.
   - local: `node "{{PLUGIN_ROOT}}/scripts/tracker.mjs" next`
...
Do not start work here. `{{CMD:tracker:start}} <id>` is the deliberate next step, so that picking and committing are separate decisions.
