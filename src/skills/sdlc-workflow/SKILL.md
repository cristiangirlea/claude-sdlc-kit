...
Every piece of work gets an id before code is written - see the `tracker` plugin (`{{CMD:tracker:pick}}`, `{{CMD:tracker:start}}`). No id means no traceability from commit to reason.
...
Run `{{CMD:sdlc:spec}}`. See the `spec-writing` skill.
...
Run `{{CMD:sdlc:plan}}`. See `task-decomposition`. Hard-to-reverse decisions become ADRs - see `adr-writing`.
...
Run `{{CMD:sdlc:implement}}`. See `tdd-workflow`. One slice at a time; parallel agents only for slices with disjoint file scopes.
...
Run `{{CMD:sdlc:review}}`. See `code-review-standards`.

### 6. Verify
**Owner:** `qa-verifier`. **Artefact:** the gate table. **Gate:** the `definition-of-done` checklist passes.

Run `{{CMD:sdlc:verify}}`. Green means observed green, not assumed.
...
Run `{{CMD:sdlc:ship}}`. See `git-workflow` and `release-management`.

### 8. Learn
**Owner:** main thread. **Artefact:** an updated `{{MEMORY}}`, a new ADR, or a tightened checklist.
...
{{MEMORY}}                        the conventions an agent must know
```
...
| Stage | Command | Agent |
| --- | --- | --- |
| Spec | `{{CMD:sdlc:spec}}` | `spec-analyst` |
| Plan | `{{CMD:sdlc:plan}}` | `solution-architect`, `codebase-explorer` |
| Implement | `{{CMD:sdlc:implement}}` | `test-author`, `task-implementer` |
| Fix a bug | `{{CMD:sdlc:bugfix}}` | `debugger`, `test-author` |
| Review | `{{CMD:sdlc:review}}` | `code-reviewer`, `security-auditor` |
| Verify | `{{CMD:sdlc:verify}}` | `qa-verifier` |
| Ship | `{{CMD:sdlc:ship}}` | `docs-scribe`, `release-manager` |
| Record a decision | `{{CMD:sdlc:adr}}` | - |
| Adopt the kit in a repo | `{{CMD:sdlc:onboard}}` | `codebase-explorer` |
| Where am I | `{{CMD:sdlc:status}}` | - |
