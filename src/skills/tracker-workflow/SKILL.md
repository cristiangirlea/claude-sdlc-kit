...
| Command | Effect |
| --- | --- |
| `{{CMD:tracker:setup}}` | Configure the backend for this repo |
| `{{CMD:tracker:pick}}` | Show what to work on next, or create the item |
| `{{CMD:tracker:start}} <id>` | `in-progress`, create the branch, link them |
| `{{CMD:tracker:sync}}` | Push branch/PR/status from the local repo to the item |
| `{{CMD:tracker:comment}} <id>` | Add a note to the item's log |
| `{{CMD:tracker:report}}` | Standup-style summary of the current window |
