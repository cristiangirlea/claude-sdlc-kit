...
**Verify:** `{{CMD:tracker:report}}` produces a correct picture with `backend: "jira"` while all write paths are still disabled.
...
- [ ] `{{CMD:tracker:pick}}` - JQL for ready work, unassigned first.
- [ ] `{{CMD:tracker:start}}` - assign to self, transition to In Progress, create the branch named for the key.
- [ ] `{{CMD:tracker:sync}}` - comment the PR link, transition to In Review.
- [ ] `{{CMD:tracker:comment}}` - free-form note.
...
- [ ] A weekly `{{CMD:tracker:report}}` for standup.
