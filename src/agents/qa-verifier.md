...
1. **Discover the real commands.** Read `Makefile`, `package.json` scripts, `justfile`, `taskfile`, CI workflow files, and `{{MEMORY}}`. Use the project's own commands - do not invent `npm test` for a Go repo.
...
user: "Is it green?"
assistant: "I will use the {{AGENT:qa-verifier}} to run build, lint and tests and report just the failures."
</example>
