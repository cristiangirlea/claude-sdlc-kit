...
```bash
node .claude/plugins/tracker/scripts/tracker.mjs list --status ready
node .claude/plugins/tracker/scripts/tracker.mjs new "Add saved searches" --type feature --priority P2
node .claude/plugins/tracker/scripts/tracker.mjs move TASK-42 in-progress
node .claude/plugins/tracker/scripts/tracker.mjs set TASK-42 branch=feat/TASK-42-saved-searches pr=123
node .claude/plugins/tracker/scripts/tracker.mjs comment TASK-42 "blocked on the auth decision"
node .claude/plugins/tracker/scripts/tracker.mjs report --days 7
```

(The path depends on where the plugin is installed; `${CLAUDE_PLUGIN_ROOT}/scripts/tracker.mjs` inside plugin-defined commands.)
