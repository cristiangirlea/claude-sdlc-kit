...
description: How to adopt this kit in an existing repository and write the project memory it depends on - discovering the real build/test/lint commands, capturing conventions into CLAUDE.md, setting permissions and hooks in .claude/settings.json, and creating the docs/ artefact layout. Use when starting work in an unfamiliar repository, when installing this kit into a project, or when CLAUDE.md is missing, stale or bloated.
---

# Onboarding a repository

The kit is generic; a project is not. Onboarding is the step that grounds generic workflow in **this** repo's real commands and conventions. Run `/sdlc:onboard` to do it interactively.
...
`CLAUDE.md` is loaded into every session in this repo. It is expensive context, so it must be **short, specific, and true**. Aim for one page.
...
Template: `templates/CLAUDE.md` in this kit.
...
- **`hooks`** - the guardrails from this kit (see `templates/settings.json`): block dangerous Bash, format on write, print branch/task context at session start.
...
Onboarding is done when you have **run** every command you wrote down and they behaved as documented. An unverified command in `CLAUDE.md` is worse than no command - it sends the next session confidently wrong.

## Keeping it true

`CLAUDE.md` rots faster than code. Update it when:
