...
Once impact is contained, use the `debugger` agent's method: reproduce, form ranked hypotheses, eliminate with evidence, prove the mechanism. **What changed?** answers most incidents: deploys, config, flags, migrations, dependency updates, traffic shape, expiring certificates and credentials, a full disk, a clock.
...
The lesson belongs where the next person will meet it: a test, a lint rule, a hook, an alert, a line in `CLAUDE.md`, or a checklist item in `definition-of-done`. An incident that changes no artefact will recur.
