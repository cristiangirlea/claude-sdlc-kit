<!--
  ...
  This file is loaded into every Claude Code session in this repo, so it is
...
This repo uses the SDLC kit: `/sdlc:status` shows where the work stands.
Specs live in `docs/specs/`, decisions in `docs/adr/`, work items in
`docs/tracker/` (or Jira - see `.sdlc/tracker.json`).
